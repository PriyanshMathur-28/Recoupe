...
Configuration lives in `.env` (see `.env.example` for where each value comes from in the Vapi dashboard). Only `VAPI_PUBLIC_KEY` is required — it is a public credential by design and is the sole value sent to the browser. `VAPI_PRIVATE_KEY` and `VAPI_PHONE_NUMBER_ID` are needed only for outbound telephony and never leave the server. `VAPI_ASSISTANT_ID`, `VAPI_VOICE_ID`, and `VAPI_WEBHOOK_SECRET` are optional. There is no simulated fallback: without a public key `resolve_mode()` returns `unconfigured`, the Call button reports "not configured", and `start_web_call` raises before a `call_log` row is opened — an outcome is only ever written from a real conversation. Point the Vapi dashboard webhook at `https://<your-host>/webhooks/vapi` and set the shared secret to the same value as `VAPI_WEBHOOK_SECRET`; when that variable is unset, webhook deliveries are rejected rather than trusted.

Dashboard-published assistants. When `VAPI_ASSISTANT_ID` is set, the backend passes it to `vapi.start()` together with `assistantOverrides.variableValues`, so an assistant authored in the Vapi dashboard may use `{{clientName}}`, `{{caseId}}`, `{{amountDue}}`, and `{{lastActivity}}`. All four keys are always sent as strings; a key the backend omitted would be read aloud to the client literally as `{{clientName}}`, so `test_every_declared_variable_is_filled` asserts the key set exactly. Without an assistant ID the backend builds a transient assistant inline from the system prompt in `modules/vapi_client.py`, and calls work the same way.
...
The outcome rule has two steps, and both closing paths (`complete_web_call` for the browser report, `normalize_end_of_call` for the server push) run it identically. Step one: silence longer than the five-second window, or a provider `endedReason` in `UNANSWERED_REASONS`, means unanswered and the outcome is `no_answer`. Step two: only if answered, the captured speech goes through a four-way classification (Groq → Gemini → deterministic heuristic), which can never return `no_answer` and escalates anything it cannot read rather than inventing a promise.

Step three is the email decision, controlled by `VOICE_AUTO_EMAIL` (default `true`). Only a `promised_to_pay` outcome may ever send: `declined`, `escalated`, and `no_answer` are refused deterministically without consulting a model at all. On a promise, a second model call reads the transcript and decides whether the conversation actually warrants the payment link — it can veto the send, and an unreachable model defaults to sending what was promised. The send runs as `resend_payment_link` through the same `handle_action` path the scheduler uses, and is audited with the actor `voice_agent` under the action `VOICE_LINK_ACTION`. Setting `VOICE_AUTO_EMAIL=false` records promises without ever minting a link. Both closing paths return the verdict to the caller as `result["email"]`, so the panel can show whether a link went out and why not when it did not (`blocked_by` is one of `outcome`, `auto_email_disabled`, `agent_declined`, `case_not_found`, `no_client_email`).

That link is deliberately invisible to attribution. `VOICE_LINK_ACTION` and `VOICE_LINK_OUTCOME` sit outside `EMAIL_SENT_OUTCOMES` and `service_layer.CASE_ACTIONS`, so `latest_email_sent_at()` does not see it and an email the call itself caused cannot steal the recovery from the call that caused it.
